 ...
 * 
 */
void CANReceive(int fifo, CAN_msg_t* CAN_rx_msg)
{
  uint32_t id = CAN1->sFIFOMailBox[fifo].RIR;
  ...
  CAN_rx_msg->len = (CAN1->sFIFOMailBox[fifo].RDTR) & 0xFUL;
  
  CAN_rx_msg->data[0] = 0xFFUL &  CAN1->sFIFOMailBox[fifo].RDLR;
  CAN_rx_msg->data[1] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDLR >> 8);
  CAN_rx_msg->data[2] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDLR >> 16);
  CAN_rx_msg->data[3] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDLR >> 24);
  CAN_rx_msg->data[4] = 0xFFUL &  CAN1->sFIFOMailBox[fifo].RDHR;
  CAN_rx_msg->data[5] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDHR >> 8);
  CAN_rx_msg->data[6] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDHR >> 16);
  CAN_rx_msg->data[7] = 0xFFUL & (CAN1->sFIFOMailBox[fifo].RDHR >> 24);
 ...
 *
 */
uint8_t CANMsgAvail(int fifo)
{
  if (fifo == 0) {
    // Check for pending FIFO 0 messages
    return CAN1->RF0R & 0x3UL;
  } else {
    // Check for pending FIFO 1 messages
    return CAN1->RF1R & 0x3UL;
  }
}

void CANPrintMessage(int fifo, CAN_msg_t CAN_RX_msg) {
  Serial.print("FIFO=");
  Serial.print(fifo);
  Serial.print(" ");
...
void setup() {
  ...
  int fbm = 0;
  bank1 = 0x0;
  bank2 = 0x00000006; // Must be IDE=0 RTR=0
  // All standard IDs are stored in FIFO 0 
  int fifo = 0;
  CANSetFilter(0, fsc, fbm, fifo, bank1, bank2);

  // One 32-Bit Filter - Identifier Mask
  bank1 = 0x04; // all extended id
  bank2 = 0x00000006; // Must be IDE=1 RTR=0
  // All extended IDs are stored in FIFO 1 
  fifo = 1;
  CANSetFilter(1, fsc, fbm, fifo, bank1, bank2);
  CAN1->FMR &= ~(0x1UL);                // Deactivate initialization mode  
}
...
void loop() {
  ...
  // receive FIFO 0
  if(CANMsgAvail(0)) {
    CANReceive(0, &CAN_RX_msg);
    CANPrintMessage(0, CAN_RX_msg);
  }
    
  // receive FIFO 1
  if(CANMsgAvail(1)) {
    CANReceive(1, &CAN_RX_msg);
    CANPrintMessage(1, CAN_RX_msg);
  }
}
