...
bool CANInit(BITRATE bitrate, int remap)
{
  ...
  RCC->APB1ENR1 |= 0x2000000UL;         // Enable CAN1 clock 
  RCC->APB1ENR1 |= 0x4000000UL;         // Enable CAN2 clock 
  ...
  if (remap == 3) {
    // CAN1
    RCC->AHB2ENR |= 0x8UL;              // Enable GPIOD clock 
    CANSetGpio(GPIOD, 0, AF9);          // Set PD0 to AF9
    CANSetGpio(GPIOD, 1, AF9);          // Set PD0 to AF9
